...
2. 二選一：
   - **全新建立**：在本資料夾執行 `clasp create --type standalone --title "演講課管理台"`，clasp 會自動產生 `.clasp.json`。（⚠️ `--type webapp` 在 clasp 3.3.0 已失效、會回 `Invalid container file type`；standalone 專案照樣部署得成 Web App。create 會覆寫 `appsscript.json`，記得 `git checkout -- appsscript.json` 還原。詳見 [SETUP.md](../SETUP.md) 第 3 步。）
